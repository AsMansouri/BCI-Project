...
for pt = 1:20
    ...
    %         [PtSegments{pt,o}, Labels, Fs, ChannInfo] = ImportingBCIData(tmpfileAdd);
            ...
            Paradigms(o).RawEpochFeatures = [reshape(PtSegments{pt,1},[size(PtSegments{pt,1},1),size(PtSegments{pt,1},2)*size(PtSegments{pt,1},3)]);...
                                            reshape(PtSegments{pt,2},[size(PtSegments{pt,2},1),size(PtSegments{pt,2},2)*size(PtSegments{pt,2},3)])];
            for E = 1:length(ERPs)
                if ERPs(E).Tag(1) =='P'
